...
if show_depuration and depuration_file:
    with st.spinner("Ejecutando depuración..."):
        try:
            filename = depuration_file.name
            depuration_file_path = f"/tmp/{filename}"
            with open(depuration_file_path, "wb") as f:
                f.write(depuration_file.read())

            # DEPURACIÓN DE AUTHORS
            try:
                sheet_name = 'Authors'
                df_authors_table = pd.read_excel(depuration_file_path, sheet_name=sheet_name)
                if df_authors_table.loc[0, 'New Author'] != '0-change-0':
                    for _, fila in df_authors_table.iterrows():
                        author = fila['Authors']
                    ...
                    st.warning("La hoja 'Authors' no ha sido completada. No se aplicaron cambios.")
            except Exception as e:
                st.error(f"Error en depuración de Authors: {e}")

            # DEPURACIÓN DE AUTHOR KEYWORDS
            try:
                sheet_name = 'Author Keywords'
                df_keywords_table = pd.read_excel(depuration_file_path, sheet_name=sheet_name)
                if df_keywords_table.loc[0, 'New Keyword'] != '0-change-0':
                    ...
                    st.warning("La hoja 'Author Keywords' no ha sido completada. No se aplicaron cambios.")
            except Exception as e:
                st.error(f"Error en depuración de Author Keywords: {e}")

            # DEPURACIÓN DE INDEX KEYWORDS
            try:
                sheet_name = 'Index Keywords'
                df_index_table = pd.read_excel(depuration_file_path, sheet_name=sheet_name)
                if df_index_table.loc[0, 'New Keyword'] != '0-change-0':
                    ...
                    st.warning("La hoja 'Index Keywords' no ha sido completada. No se aplicaron cambios.")
            except Exception as e:
                st.error(f"Error en depuración de Index Keywords: {e}")

            # DEPURACIÓN DE CITED REFERENCES
            try:
                sheet_name = 'Cited References'
                df_ref_table = pd.read_excel(depuration_file_path, sheet_name=sheet_name)
                df_ref_table.fillna('', inplace=True)
                if df_ref_table.loc[0, 'New Reference'] != '0-change-0':
                    ...
                    df_final['References'] = df_final['References'].apply(lambda x: '; '.join([k.strip() for k in x.split(';')]))
                    df_final['References'] = df_final['References'].str.replace(" ;", "")
                    st.success("Depuración de Cited References completada correctamente.")
                else:
                    st.warning("La hoja 'Cited References' no ha sido completada. No se aplicaron cambios.")
            except Exception as e:
                st.error(f"Error en depuración de Cited References: {e}")

        except Exception as e:
            st.error(f"Error general durante la depuración: {e}")
