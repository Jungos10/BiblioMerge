...
# === INICIO PARTE 3 ===
st.markdown("### Paso opcional: Depuración de campos con archivo Excel externo")
depuracion_activa = st.checkbox("¿Deseas aplicar depuración personalizada con un archivo de conversión?")

if depuracion_activa:
    archivo_excel_conversion = st.file_uploader("Selecciona el archivo Excel con las hojas de conversión", type=["xlsx"])

    if archivo_excel_conversion is not None:
        try:
            excel_data = pd.read_excel(archivo_excel_conversion, sheet_name=None)

            for sheet_name in ['Authors', 'Author Keywords', 'Index Keywords', 'Cited References']:
                if sheet_name not in excel_data:
                    st.warning(f"La hoja '{sheet_name}' no está presente en el Excel.")
                    continue

                df_sheet = excel_data[sheet_name]

                if df_sheet.empty:
                    st.warning(f"Depuración de {sheet_name} no posible: la hoja está vacía.")
                    continue

                campo_nuevo = 'New Author' if sheet_name == 'Authors' else 'New Keyword' if 'Keyword' in sheet_name else 'New Reference'
                if df_sheet.loc[0, campo_nuevo] == '0-change-0':
                    st.warning(f"La hoja '{sheet_name}' no ha sido completada. No se aplicaron cambios.")
                    continue

                for _, fila in df_sheet.iterrows():
                    origen = fila[sheet_name if sheet_name != 'Cited References' else 'References']
                    destino = fila[campo_nuevo]

                    if sheet_name == 'Authors':
                        fila_ref = autores[autores['Authors'] == origen]
                        if not fila_ref.empty:
                            idx = [int(i) for i in fila_ref['Indices'].iloc[0].split(';')]
                            pos = [int(p) for p in fila_ref['Posiciones'].iloc[0].split(';')]
                            for i, p in zip(idx, pos):
                                partes = df_final.at[i, 'Authors'].split(';')
                                if p < len(partes):
                                    partes[p] = destino
                                    df_final.at[i, 'Authors'] = '; '.join(partes)
                        df_final['Authors'] = df_final['Authors'].apply(lambda x: '; '.join([s.strip() for s in x.split(';')]))
                        df_final['Author full names'] = df_final['Authors']

                    elif sheet_name in ['Author Keywords', 'Index Keywords']:
                        df_ref = df_author_keywords if sheet_name == 'Author Keywords' else df_index_keywords
                        fila_ref = df_ref[df_ref[sheet_name] == origen]
                        if not fila_ref.empty:
                            idx = [int(i) for i in fila_ref['Indices'].iloc[0].split(';')]
                            pos = [int(p) for p in fila_ref['Posiciones'].iloc[0].split(';')]
                            for i, p in zip(idx, pos):
                                partes = df_final.at[i, sheet_name].split(';')
                                if p < len(partes):
                                    partes[p] = destino
                                    df_final.at[i, sheet_name] = '; '.join(partes)
                            df_final[sheet_name] = df_final[sheet_name].apply(lambda x: '; '.join([s.strip() for s in x.split(';')]))

                    elif sheet_name == 'Cited References':
                        fila_ref = df_references_info[df_references_info['References'] == origen]
                        if not fila_ref.empty:
                            idx = [int(i) for i in fila_ref['Indices'].iloc[0].split(';')]
                            pos = [int(p) for p in fila_ref['Posiciones'].iloc[0].split(';')]
                            for i, p in zip(idx, pos):
                            ...
                            df_final['References'] = df_final['References'].apply(lambda x: '; '.join([s.strip() for s in x.split(';')]))
                            df_final['References'] = df_final['References'].str.replace(" ;", "")

                st.success(f"Depuración de '{sheet_name}' completada correctamente.")
        except Exception as e:
