...
if st.button("📁 Generar ficheros finales"):
    ...
    plt.xticks(rotation=90)
    st.pyplot(fig4)
    # Parte 3: Depuración opcional de autores/keywords/referencias
    st.header("🔷 Parte 3: Depuración opcional del usuario")

    realizar_depuracion = st.checkbox("🔍 Realizar depuración manual de autores/keywords/referencias")

    if realizar_depuracion:
        st.markdown("Puedes subir el archivo Excel con las hojas correspondientes para aplicar los cambios.")
        tabla_conversion = st.file_uploader("📤 Sube el Excel de conversión (Autores, Keywords, Referencias)", type=["xlsx"])

        if tabla_conversion and st.button("✅ Aplicar depuración"):
            import pandas as pd

            try:
                df_authors_table = pd.read_excel(tabla_conversion, sheet_name="Authors")
                ...
                if primer_nuevo_author != "0-change-0":
                    for _, fila in df_authors_table.iterrows():
                        ...
                        if not fila_encontrada.empty:
                            indices_str = fila_encontrada["Indices"].iloc[0]
                            posiciones_str = fila_encontrada["Posiciones"].iloc[0]
                            indices_encontrados = [int(i) for i in indices_str.split(";")]
                            posiciones_encontradas = [int(i) for i in posiciones_str.split(";")]
                            for index, position in zip(indices_encontrados, posiciones_encontradas):
                                if index in df_final.index:
                                    author_list = df_final.at[index, "Authors"].split(";")
                                    if 0 <= position < len(author_list):
                                        author_list[position] = nueva_author
                                        df_final.at[index, "Authors"] = "; ".join(author_list)
                    df_final["Authors"] = df_final["Authors"].apply(lambda x: "; ".join([i.strip() for i in x.split(";")]))
                    df_final["Author full names"] = df_final["Authors"]
                    st.success("Depuración de Authors completada correctamente.")
            except Exception as e:
                st.warning(f"Error en depuración de Authors: {e}")

            # Author Keywords
            try:
                df_ak_table = pd.read_excel(tabla_conversion, sheet_name="Author Keywords")
                primer_kw = df_ak_table.loc[0, "New Keyword"]
                if primer_kw != "0-change-0":
                    for _, fila in df_ak_table.iterrows():
                        ak = fila["Author Keyword"]
                        new_kw = fila["New Keyword"]
                        fila_encontrada = df_author_keywords[df_author_keywords["Author Keyword"] == ak]
                        if not fila_encontrada.empty:
                            ...
                            for index, position in zip(indices, posiciones):
                                if index in df_final.index:
                                    kws = df_final.at[index, "Author Keywords"].split(";")
                                    if 0 <= position < len(kws):
                                        kws[position] = new_kw
                                        df_final.at[index, "Author Keywords"] = "; ".join(kws)
                    df_final["Author Keywords"] = df_final["Author Keywords"].apply(lambda x: "; ".join([i.strip() for i in x.split(";")]))
                    st.success("Depuración de Author Keywords completada correctamente.")
            except Exception as e:
                st.warning(f"Error en depuración de Author Keywords: {e}")

            # Index Keywords
            try:
                df_ik_table = pd.read_excel(tabla_conversion, sheet_name="Index Keywords")
                primer_kw = df_ik_table.loc[0, "New Keyword"]
                if primer_kw != "0-change-0":
                    for _, fila in df_ik_table.iterrows():
                        ik = fila["Index Keywords"]
                        new_kw = fila["New Keyword"]
                        fila_encontrada = df_index_keywords[df_index_keywords["Index Keywords"] == ik]
                        if not fila_encontrada.empty:
                            indices = [int(i) for i in fila_encontrada["Indices"].iloc[0].split(";")]
                            posiciones = [int(p) for p in fila_encontrada["Posiciones"].iloc[0].split(";")]
                            for index, position in zip(indices, posiciones):
                                if index in df_final.index:
                                    kws = df_final.at[index, "Index Keywords"].split(";")
                                    if 0 <= position < len(kws):
                                        kws[position] = new_kw
                                        df_final.at[index, "Index Keywords"] = "; ".join(kws)
                    df_final["Index Keywords"] = df_final["Index Keywords"].apply(lambda x: "; ".join([i.strip() for i in x.split(";")]))
                    st.success("Depuración de Index Keywords completada correctamente.")
            except Exception as e:
                st.warning(f"Error en depuración de Index Keywords: {e}")

            # References
            try:
                df_ref_table = pd.read_excel(tabla_conversion, sheet_name="Cited References")
                primer_ref = df_ref_table.loc[0, "New Reference"]
                if primer_ref != "0-change-0":
                    for _, fila in df_ref_table.iterrows():
                        ref = fila["References"]
                        ...
                        if not fila_encontrada.empty:
                            ...
                            for index, position in zip(indices, posiciones):
                                if index in df_final.index:
                                    refs = df_final.at[index, "References"].split(";")
                                    if 0 <= position < len(refs):
                                        refs[position] = new_ref
                                        df_final.at[index, "References"] = "; ".join(refs)
                    df_final["References"] = df_final["References"].apply(lambda x: "; ".join([i.strip() for i in x.split(";")]).replace(" ;", ""))
                    st.success("Depuración de Cited References completada correctamente.")
            except Exception as e:
                st.warning(f"Error en depuración de Cited References: {e}")
