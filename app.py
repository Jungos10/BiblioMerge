...
uploaded_files_scopus = st.file_uploader(
    "Selecciona uno o varios archivos de Scopus (CSV)",
    type=['csv'],
    accept_multiple_files=True
)

dfsco_list = []
if uploaded_files_scopus:
    for file in uploaded_files_scopus:
        df = pd.read_csv(file)
...
st.subheader('Carga de archivos TXT de WOS')
uploaded_files_wos = st.file_uploader(
    "Selecciona uno o varios archivos de WOS (TXT)",
    type=['txt'],
    accept_multiple_files=True
)

dfwos_list = []
if uploaded_files_wos:
    for file in uploaded_files_wos:
        df = pd.read_csv(file, sep='\t', encoding='utf-8')
        dfwos_list.append(df)
    dfwos = pd.concat(dfwos_list, ignore_index=True)
    st.success('Archivos de WOS cargados y concatenados correctamente.')
