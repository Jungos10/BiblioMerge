...
if st.button("📁 Generar ficheros finales"):
    ...
    st.pyplot(fig4)
    # -------------------- DEPURACIÓN OPCIONAL ------------------------------
    st.markdown("### Opcional: Depurar autores, keywords y referencias desde Excel")
    activar_depuracion = st.checkbox("Activar depuración por tabla Excel")
    if activar_depuracion:
        uploaded_excel = st.file_uploader("Carga el archivo Excel con las tablas de conversión", type=["xlsx"], key="depuracion_excel")
        if uploaded_excel is not None:
            if st.button("Ejecutar depuración"):
                import pandas as pd
                try:
                    excel_data = pd.ExcelFile(uploaded_excel)

                    # DEPURACIÓN AUTHORS
                    if 'Authors' in excel_data.sheet_names:
                        df_authors_table = pd.read_excel(excel_data, sheet_name='Authors')
                        if not df_authors_table.empty and df_authors_table.loc[0, 'New Author'] != '0-change-0':
                            for _, fila in df_authors_table.iterrows():
                                ...
                                if not fila_encontrada.empty:
                                    ...
                                    posiciones_encontradas = [int(p) for p in posiciones_str.split(';')]
                                    for index, position in zip(indices_encontrados, posiciones_encontradas):
                                        if index in df_final.index:
                                            partes = df_final.at[index, 'Authors'].split(';')
                                            if position < len(partes):
                                                partes[position] = nueva_author
                                                df_final.at[index, 'Authors'] = '; '.join(partes)
                    ...
                    if 'Author Keywords' in excel_data.sheet_names:
                        df_keywords_table = pd.read_excel(excel_data, sheet_name='Author Keywords')
                        if not df_keywords_table.empty and df_keywords_table.loc[0, 'New Keyword'] != '0-change-0':
                            for _, fila in df_keywords_table.iterrows():
                                keyword = fila['Author Keyword']
                                nueva = fila['New Keyword']
                                ...
                                if not fila_encontrada.empty:
                                    ...
                                    for i, pos in zip(indices, posiciones):
                                        if i in df_final.index:
                                            partes = df_final.at[i, 'Author Keywords'].split(';')
                                            if pos < len(partes):
                                                partes[pos] = nueva
                                                df_final.at[i, 'Author Keywords'] = '; '.join(partes)
                    ...
                    if 'Index Keywords' in excel_data.sheet_names:
                        df_index_table = pd.read_excel(excel_data, sheet_name='Index Keywords')
                        if not df_index_table.empty and df_index_table.loc[0, 'New Keyword'] != '0-change-0':
                            for _, fila in df_index_table.iterrows():
                                ...
                                if not fila_encontrada.empty:
                                    ...
                                    for i, pos in zip(indices, posiciones):
                                        if i in df_final.index:
                                            partes = df_final.at[i, 'Index Keywords'].split(';')
                                            if pos < len(partes):
                                                partes[pos] = nueva
                                                df_final.at[i, 'Index Keywords'] = '; '.join(partes)
                    ...
                    # DEPURACIÓN CITED REFERENCES
                    if 'Cited References' in excel_data.sheet_names:
                        df_references_table = pd.read_excel(excel_data, sheet_name='Cited References')
                        if not df_references_table.empty and df_references_table.loc[0, 'New Reference'] != '0-change-0':
                            df_references_table.fillna('', inplace=True)
                            for _, fila in df_references_table.iterrows():
                                ref = fila['References']
                                nueva = fila['New Reference']
                                ...
                                if not fila_encontrada.empty:
                                    ...
                                    for i, pos in zip(indices, posiciones):
                                        if i in df_final.index:
                                            partes = df_final.at[i, 'References'].split(';')
                                            if pos < len(partes):
                                                partes[pos] = nueva
                                                df_final.at[i, 'References'] = '; '.join(partes)
                    ...
                    st.error(f"Ocurrió un error durante la depuración: {str(e)}")
