...
if st.button("📁 Generar ficheros finales"):
    ...
    plt.xticks(rotation=90)
    st.pyplot(fig4)
    # ---------------------- OPCIONAL: DEPURACIÓN CON ARCHIVO EXTERNO ----------------------
    with st.expander("🔍 Depuración opcional de autores, keywords y referencias"):
        activar_depuracion = st.checkbox("Quiero hacer una depuración con tabla externa")
        if activar_depuracion:
            uploaded_excel = st.file_uploader("Carga el Excel con las tablas de conversión", type=["xlsx"], key="depuracion_excel")
            if uploaded_excel:
                try:
                    # DEPURACIÓN DE AUTHORS
                    df_authors_table = pd.read_excel(uploaded_excel, sheet_name='Authors')
                    if not df_authors_table.empty and df_authors_table['New Author'].iloc[0] != '0-change-0':
                        for _, fila in df_authors_table.iterrows():
                            author = fila['Authors']
                            new_author = fila['New Author']
                            fila_encontrada = autores[autores['Authors'] == author]
                            if not fila_encontrada.empty:
                                indices = list(map(int, fila_encontrada['Indices'].iloc[0].split(';')))
                                posiciones = list(map(int, fila_encontrada['Posiciones'].iloc[0].split(';')))
                                for i, pos in zip(indices, posiciones):
                                    if i in df_final.index:
                                        autores_actuales = df_final.at[i, 'Authors'].split(';')
                                        if pos < len(autores_actuales):
                                            autores_actuales[pos] = new_author
                                            df_final.at[i, 'Authors'] = '; '.join(autores_actuales)
                        ...
                        st.success("Depuración de Authors completada.")

                    # DEPURACIÓN DE AUTHOR KEYWORDS
                    df_authors_keywords_table = pd.read_excel(uploaded_excel, sheet_name='Author Keywords')
                    if not df_authors_keywords_table.empty and df_authors_keywords_table['New Keyword'].iloc[0] != '0-change-0':
                        for _, fila in df_authors_keywords_table.iterrows():
                            key = fila['Author Keyword']
                            new_key = fila['New Keyword']
                            fila_encontrada = df_author_keywords[df_author_keywords['Author Keyword'] == key]
                            if not fila_encontrada.empty:
                                indices = list(map(int, fila_encontrada['Indices'].iloc[0].split(';')))
                                posiciones = list(map(int, fila_encontrada['Posiciones'].iloc[0].split(';')))
                                for i, pos in zip(indices, posiciones):
                                    if i in df_final.index:
                                        valores = df_final.at[i, 'Author Keywords'].split(';')
                                        if pos < len(valores):
                                            valores[pos] = new_key
                                            df_final.at[i, 'Author Keywords'] = '; '.join(valores)
                        df_final['Author Keywords'] = df_final['Author Keywords'].apply(lambda x: '; '.join([k.strip() for k in x.split(';')]))
                        st.success("Depuración de Author Keywords completada.")

                    # DEPURACIÓN DE INDEX KEYWORDS
                    df_index_keywords_table = pd.read_excel(uploaded_excel, sheet_name='Index Keywords')
                    if not df_index_keywords_table.empty and df_index_keywords_table['New Keyword'].iloc[0] != '0-change-0':
                        for _, fila in df_index_keywords_table.iterrows():
                            key = fila['Index Keywords']
                            new_key = fila['New Keyword']
                            fila_encontrada = df_index_keywords[df_index_keywords['Index Keywords'] == key]
                            if not fila_encontrada.empty:
                                indices = list(map(int, fila_encontrada['Indices'].iloc[0].split(';')))
                                posiciones = list(map(int, fila_encontrada['Posiciones'].iloc[0].split(';')))
                                for i, pos in zip(indices, posiciones):
                                    if i in df_final.index:
                                        valores = df_final.at[i, 'Index Keywords'].split(';')
                                        if pos < len(valores):
                                            valores[pos] = new_key
                                            df_final.at[i, 'Index Keywords'] = '; '.join(valores)
                        df_final['Index Keywords'] = df_final['Index Keywords'].apply(lambda x: '; '.join([k.strip() for k in x.split(';')]))
                        st.success("Depuración de Index Keywords completada.")

                    # DEPURACIÓN DE CITED REFERENCES
                    df_refs_table = pd.read_excel(uploaded_excel, sheet_name='Cited References')
                    if not df_refs_table.empty and df_refs_table['New Reference'].iloc[0] != '0-change-0':
                        df_refs_table.fillna('', inplace=True)
                        for _, fila in df_refs_table.iterrows():
                            ...
                            if not fila_encontrada.empty:
                                ...
                                for i, pos in zip(indices, posiciones):
                                    if i in df_final.index:
                                        valores = df_final.at[i, 'References'].split(';')
                                        if pos < len(valores):
                                            valores[pos] = new_ref
                                            df_final.at[i, 'References'] = '; '.join(valores)
                        df_final['References'] = df_final['References'].apply(lambda x: '; '.join([r.strip() for r in x.split(';')]))
                        df_final['References'] = df_final['References'].str.replace(" ;", "", regex=False)
                        st.success("Depuración de Cited References completada.")

                except Exception as e:
                    st.error(f"Ocurrió un error al procesar el archivo de depuración: {e}")
